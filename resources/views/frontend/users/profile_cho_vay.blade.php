        ...
        <div class="tm-account bg-white p-md-5 pt-md-4 p-3">
                ...
                {{ Form::open(['route' => ['frontend.user.save_service', $data->id], 'method' => 'POST']) }}
                        ...
                        <div class="row">
                            @foreach( \Common::getListServices() as $key => $value )
                                <label class="custom-control col-md-5">
                                    <input value="{{ $key }}" name="servies[]" {{ in_array($key, $data->services()) ? 'checked' : '' }} type="checkbox" class="custom-control-input">
                    ...
                    <div class="form-group">
                        <div class="text-center">
                            <button type="submit" class="btn btn-primary" onclick="UpdateSpices()">CẬP NHẬT GÓI SẢN PHẨM MỚI</button>
                        </div>
                    </div>
                {{ Form::close() }}
            </div>
            ...
            <div class="tm-dtcv bg-white p-3 px-md-5 pb-md-5 pt-md-4" id="editDistrictSpice">
                ...
                {{ Form::open(['route' => ['frontend.user.save_location', $data->id], 'method' => 'POST']) }}
                    ...
                    <div class="districtDiv form-group row">
                        ...
                        <div class="col-sm-3">
                            <label class="custom-control fs-13 mr-5">
                                <input value="{{ $value['tid'] }}" name="districts[]" {{ in_array($value['tid'], $data->locations()) ? 'checked' : '' }} type="checkbox" class="custom-control-input">
                                <span class="custom-control-indicator"></span>
                                <span class="custom-control-description" style="font-size: 17px">{{ $value['name'] }}</span>
                            </label>
                        </div>
                        @endforeach
                    </div>
                    <div class="form-group">
                        <div class="text-center">
                            <button type="submit" class="btn btn-primary" onclick="UpdateDistrictSpices()">CẬP NHẬT TỈNH THÀNH</button>
                        </div>
                    </div>
                {{ Form::close() }}
            </div>
